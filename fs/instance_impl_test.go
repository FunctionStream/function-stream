...
func TestNewFunctionInstance(t *testing.T) {
	...
	if ctxValue, ok := Instance.ctx.Value("function-name").(string); !ok || ctxValue != definition.Name {
		t.Errorf("Expected 'function-name' in ctx to be '%s'", definition.Name)
	}

	if ctxValue, ok := Instance.ctx.Value("function-index").(int32); !ok || ctxValue != index {
