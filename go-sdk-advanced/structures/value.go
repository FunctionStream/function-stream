...
// Package structures provides state types (ValueState, ListState, MapState, etc.)
// for the Advanced SDK. Depends on go-sdk (low-level) and go-sdk-advanced/codec.
package structures
...
func newValueState[T any](store common.Store, valueCodec codec.Codec[T]) (*ValueState[T], error) {
	...
	return &ValueState[T]{store: store, complexKey: ck, valueCodec: valueCodec}, nil
}
...
func (v *ValueState[T]) Update(value T) error {
	encoded, err := v.valueCodec.Encode(value)
	if err != nil {
...
func (v *ValueState[T]) Value() (T, bool, error) {
	...
	decoded, err := v.valueCodec.Decode(raw)
	if err != nil {
