...
use crate::sql::statement::{Statement, CreateWasmTask, DropWasmTask, ShowWasmTasks, StartWasmTask, StopWasmTask};
    ...
    pub fn parse(sql: &str) -> Result<Box<dyn Statement>, ParseError> {
        ...
        for pair in pairs {
            return match pair.as_rule() {
                Rule::create_stmt  => handle_create_stmt(pair).map(|stmt| stmt as Box<dyn Statement>),
...
fn handle_create_stmt(pair: pest::iterators::Pair<Rule>) -> Result<Box<CreateWasmTask>, ParseError> {
    ...
    for prop in pair.into_inner() {
        if prop.as_rule() == Rule::property {
            ...
            if let (Some(key_pair), Some(val_pair)) = (inner.next(), inner.next()) {
                let key = key_pair.into_inner().next()
                    .map(extract_string)
                    .unwrap_or_default();
                let value = val_pair.into_inner().next()
                    .map(extract_string)
                    .unwrap_or_default();
