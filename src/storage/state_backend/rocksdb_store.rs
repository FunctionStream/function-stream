...
use crate::storage::state_backend::store::{StateStore, StateIterator};
    ...
    fn put_cf(&self, key: &[u8], value: &[u8]) -> Result<(), BackendError> {
        if let Some(ref cf_name) = self.column_family_name {
            let cf = self.db.cf_handle(cf_name)
                .ok_or_else(|| BackendError::Other(format!("Column family '{}' not found", cf_name)))?;
...
impl StateStore for RocksDBStateStore {
    ...
    fn list_states(&self, start_inclusive: Vec<u8>, end_exclusive: Vec<u8>) -> Result<Vec<Vec<u8>>, BackendError> {
        ...
        let iter = if let Some(ref cf_name) = self.column_family_name {
            let cf = self.db.cf_handle(cf_name)
                .ok_or_else(|| BackendError::Other(format!("Column family '{}' not found", cf_name)))?;
            self.db.iterator_cf(&cf, IteratorMode::From(&start_inclusive, rocksdb::Direction::Forward))
        } else {
            self.db.iterator(IteratorMode::From(&start_inclusive, rocksdb::Direction::Forward))
        };
        
        for item in iter {
            let (key, _) = item.map_err(|e| BackendError::IoError(format!("RocksDB iterator error: {}", e)))?;
    ...
    fn delete_prefix_bytes(&self, prefix: Vec<u8>) -> Result<usize, BackendError> {
        ...
        if is_all_0xff(&prefix) {
            ...
            let iter = if let Some(ref cf_name) = self.column_family_name {
                let cf = self.db.cf_handle(cf_name)
                    .ok_or_else(|| BackendError::Other(format!("Column family '{}' not found", cf_name)))?;
                self.db.iterator_cf(&cf, IteratorMode::From(&prefix, rocksdb::Direction::Forward))
            } else {
            ...
            if count > 0 {
                self.db.write(batch)
                    .map_err(|e| BackendError::IoError(format!("RocksDB batch write error: {}", e)))?;
            }
...
impl RocksDBStateIterator {
    ...
    fn find_next(&self) -> Result<Option<(Vec<u8>, Vec<u8>)>, BackendError> {
        let last_key = self.last_key.lock()
        ...
        // 查找下一个匹配前缀的项
        for item in iter {
            let (key, value) = item.map_err(|e| BackendError::IoError(format!("RocksDB iterator error: {}", e)))?;
            let key_slice = key.as_ref();
            
            // 跳过上次的键（如果存在）
            if let Some(ref last) = *last_key {
                if key_slice <= last.as_slice() {
                    continue;
                }
            }
