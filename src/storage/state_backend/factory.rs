...
pub trait StateStoreFactory: Send + Sync {
    ...
    fn new_state_store(&self, column_family: Option<String>) -> Result<Box<dyn StateStore>, BackendError>;
}
...
pub fn get_factory_for_task<P: AsRef<Path>>(
    ...
    match factory_type {
        ...
        FactoryType::RocksDB => {
            ...
            let db_path = base_dir.as_ref()
                .join(format!("{}-{}", task_name, created_at));
            
            // 使用提供的配置，如果没有则使用默认配置
            let config = rocksdb_config.unwrap_or_default();
            let factory = crate::storage::state_backend::rocksdb_factory::RocksDBStateStoreFactory::new(
                db_path,
                config,
            )?;
