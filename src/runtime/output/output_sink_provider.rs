...
impl OutputSinkProvider {
    ...
    pub fn from_output_configs(
        ...
        if output_configs.len() > MAX_OUTPUTS {
            return Err(Box::new(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("Too many outputs: {} (maximum is {})", output_configs.len(), MAX_OUTPUTS),
            )) as Box<dyn std::error::Error + Send>);
        }
    ...
    fn from_output_config(
        ...
        match output_config {
            OutputConfig::Kafka { 
                ...
                use crate::runtime::output::protocol::kafka::{KafkaProducerConfig, KafkaOutputSink};
                ...
                if servers.is_empty() {
                    return Err(Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                        format!("Invalid bootstrap_servers in output config: empty or invalid (topic: {})", topic),
                    )) as Box<dyn std::error::Error + Send>);
                }
                ...
                let kafka_config = KafkaProducerConfig::new(
                    servers,
                    topic.clone(),
                    partition_opt,
                    properties,
                ...
                Ok(Box::new(KafkaOutputSink::from_config(kafka_config, sink_idx)))
            }
        }
    }
}
