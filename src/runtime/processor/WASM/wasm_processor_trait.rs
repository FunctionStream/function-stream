...
pub trait WasmProcessor: Send + Sync {
    ...
    fn process(&self, data: Vec<u8>, input_index: usize) -> Result<(), Box<dyn std::error::Error + Send>>;
    ...
    fn process_watermark(&mut self, timestamp: u64, input_index: usize) -> Result<(), Box<dyn std::error::Error + Send>> {
        // Default implementation: do nothing
        log::debug!("Processing watermark: {} from input {}", timestamp, input_index);
        Ok(())
    }
    ...
    fn take_checkpoint_sinks(&mut self, checkpoint_id: u64) -> Result<(), Box<dyn std::error::Error + Send>> {
        // Default implementation: do nothing
    ...
    fn finish_checkpoint_sinks(&mut self, checkpoint_id: u64) -> Result<(), Box<dyn std::error::Error + Send>> {
        // Default implementation: do nothing
