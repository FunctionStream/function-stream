...
use crate::runtime::task::yaml_keys::{TYPE, NAME, type_values};
...
impl TaskBuilder {
    ...
    pub fn from_yaml_config(
        ...
        log::debug!("TaskBuilder::from_yaml_config: config size={} bytes, wasm size={} bytes", 
            config_bytes.len(), wasm_bytes.len());
        
        // 1. Parse YAML configuration
        log::debug!("Step 1: Parsing YAML config");
        let yaml_value: Value = serde_yaml::from_slice(config_bytes)
            .map_err(|e| -> Box<dyn std::error::Error + Send> {
                ...
                log::error!("Failed to parse YAML config. Error: {}. Config preview (first 500 chars):\n{}", e, preview);
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("Failed to parse YAML config: {}. Config preview: {}", e, preview)
                ))
            })?;
        ...
        let task_name = yaml_value
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                let available_keys: Vec<String> = yaml_value.as_mapping()
                    .map(|m| m.keys().filter_map(|k| k.as_str().map(|s| s.to_string())).collect())
                    .unwrap_or_default();
                log::error!("Missing '{}' field in YAML config. Available keys: {:?}", NAME, available_keys);
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("Missing '{}' field in YAML config. Available keys: {:?}", NAME, available_keys),
        ...
        let config_type_str = yaml_value
            ...
            .and_then(|v| v.as_str())
            .ok_or_else(|| {
                log::error!("Missing '{}' field in YAML config for task '{}'", TYPE, task_name);
                Box::new(std::io::Error::new(
                    std::io::ErrorKind::InvalidData,
                    format!("Missing '{}' field in YAML config for task '{}'", TYPE, task_name),
                )) as Box<dyn std::error::Error + Send>
            })?;
        ...
        log::debug!("Step 5: Building task based on config type '{}'", config_type_str);
        let task: Box<dyn TaskLifecycle> = match config_type_str {
            type_values::PROCESSOR => {
                log::debug!("Building processor task '{}'", task_name);
                let wasm_task = ProcessorBuilder::build(task_name.clone(), &yaml_value, wasm_path_str)
                    .map_err(|e| -> Box<dyn std::error::Error + Send> {
                        log::error!("ProcessorBuilder::build failed for task '{}': {}", task_name, e);
                        e
                    })?;
            ...
            type_values::SOURCE => {
                let wasm_task = SourceBuilder::build(task_name.clone(), &yaml_value, wasm_path_str)?;
                Box::new(Arc::try_unwrap(wasm_task)
