...
use super::varint::{encode_var_int32, decode_var_int32, compute_var_int32_size};
...
pub fn encode_string(buffer: &mut [u8], offset: usize, s: &str) -> Result<usize, Box<dyn std::error::Error + Send>> {
...
pub fn decode_string(bytes: &[u8], offset: usize) -> Result<(String, usize), Box<dyn std::error::Error + Send>> {
...
/// Writes to the specified position in the buffer, returns the number of bytes written
pub fn encode_byte_string(buffer: &mut [u8], offset: usize, bytes: &[u8]) -> Result<usize, Box<dyn std::error::Error + Send>> {
    let length = bytes.len() as i32;
    let length_size = compute_var_int32_size(length);
...
/// Decodes from the specified position in the byte array, returns (decoded value, bytes consumed)
pub fn decode_byte_string(bytes: &[u8], offset: usize) -> Result<(Vec<u8>, usize), Box<dyn std::error::Error + Send>> {
    let (length, length_consumed) = decode_var_int32(bytes, offset)?;
    let length = length as usize;
    let start = offset + length_consumed;
