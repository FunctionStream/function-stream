...
def build_wasm(componentize_cmd, deps_dir):
    ...
    # 检查主模块文件（fs_runtime.runner 作为入口点）
    main_module_file = SCRIPT_DIR / "src" / "fs_runtime" / "runner.py"
    ...
    # 使用 fs_runtime.runner 作为主模块（包含 WitWorld 类）
    main_module_name = "fs_runtime.runner"
    
    # 构建完整命令
    # 使用 --python-path (-p) 指定多个路径：
    # 1. dependencies 目录（包含 fs_api）
    # 2. 当前目录（包含 main.py 和 fs_runtime）
    cmd = componentize_cmd + [
        ...
        "-p", str(SCRIPT_DIR / "src"),    # Python 路径 2：src 目录（包含 fs_runtime）
